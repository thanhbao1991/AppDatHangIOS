import SwiftUI

/// Thay cho `Label(_:systemImage:)` — icon là emoji thay vì SF Symbol (đổi 2026-09-13, đồng bộ
/// phong cách emoji đã dùng khắp app). Layout giống Label mặc định: icon + chữ cạnh nhau, icon có
/// width cố định để các dòng cùng chỗ thẳng hàng dù emoji rộng hẹp khác nhau.
struct EmojiLabel: View {
    let title: String
    let icon: String

    init(_ title: String, _ icon: String) {
        self.title = title
        self.icon = icon
    }

    var body: some View {
        HStack(spacing: 10) {
            Text(icon).font(.system(size: 15)).frame(width: 20)
            Text(title)
        }
    }
}
